...
export default function SoldForm({ onSave, initial }) {
    ...
    const [thumb, setThumb] = useState(initial?.thumb || null);
        ...
        <form onSubmit={submit} className="grid gap-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Field label="Painting Title" required>
                    <TextInput value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g., Evening Garden" />
                </Field>
                <Field label="Price (USD)" required>
                    <TextInput inputMode="decimal" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="e.g., 650" />
                </Field>
                <Field label="Buyer">
                    <TextInput value={buyer} onChange={(e) => setBuyer(e.target.value)} placeholder="Name or gallery" />
                </Field>
